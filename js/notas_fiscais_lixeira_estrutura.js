...
function atualizarBadgeLixeira() {
    const count = (typeof lixeira !== 'undefined' && Array.isArray(lixeira)) ? lixeira.length : 0;
    ...
    var headerBadge = document.getElementById('lixeira-header-badge');
    if (headerBadge) {
        headerBadge.textContent = count;
        headerBadge.style.display = count === 0 ? 'none' : 'inline-flex';
    }
}
...
function mostrarLixeira() {
    ...
    // Atualizar menu ativo (menu antigo e novo)
    ...
    var lixeiraNav = document.querySelector('.nav-item[onclick*="mostrarLixeira"]') ||
                     document.querySelector('.sidebar-quick-trash');
    if (lixeiraNav) {
    ...
    if (titleEl) titleEl.textContent = 'Lixeira';
    if (subtitleEl) subtitleEl.textContent = 'Notas fiscais excluídas';
