...
function getTotpLoginNormalized() {
    ...
    if (raw != null && typeof raw === 'string' && raw.trim() !== '') {
        return String(raw).trim().toLowerCase().replace(/\s+/g, '_').replace(/\./g, '_');
    }
    raw = localStorage.getItem('current_user');
    if (raw != null && typeof raw === 'string' && raw.trim() !== '') {
        return String(raw).trim().toLowerCase().replace(/\s+/g, '_').replace(/\./g, '_');
    }
    return '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('beforeunload', (e) => {
        ...
        if (page && page.startsWith('page-')) localStorage.setItem('axis-current-page', page);
    ...
    // Se forceLogin, NÃO restaurar página salva (mantém #login)
    const savedPage = forceLogin ? null : localStorage.getItem('axis-current-page');
    const hashPage = (window.location.hash || '').replace('#', '').trim();
    let pageToUse = (hashPage && hashPage.startsWith('page-')) ? hashPage : (savedPage && savedPage.startsWith('page-') ? savedPage : 'page-home');
    if (forceLogin) {
        pageToUse = 'page-home';
        localStorage.setItem('axis-current-page', 'page-home');
    } else if (hashPage && hashPage.startsWith('page-')) {
        pageToUse = hashPage;
    ...
    if (savedUser && finalIsLoggedIn) {
        ...
        function showMainContentRestore() {
            ...
            setTimeout(function() { if (typeof window.axisRobotMaybeShow === 'function') try { window.axisRobotMaybeShow(); } catch (_) {} }, 600);
            var introOverlay = document.getElementById('intro-axis-overlay');
            ...
            if (!wasJustLoggedIn) {
                ...
                if (savedPage && savedPage.startsWith('page-')) {
                    ...
                    setTimeout(() => {
                        if (typeof navigate === 'function') {
                            try { navigate(savedPage); } catch (e) { navigateToPage(savedPage); }
                    ...
                    const goHome = function() {
                        var homeSection = document.getElementById(defaultPage);
                        if (homeSection) {
                            document.querySelectorAll('.main-section').forEach(s => {
                                s.classList.remove('active');
                                s.style.display = 'none';
                    ...
                    setTimeout(() => {
                        if (typeof navigate === 'function') { try { navigate(defaultPage); } catch (e) { goHome(); } }
                        else { goHome(); }
                    }, 100);
                }
            } else {
                ...
                setTimeout(() => {
                    if (typeof navigate === 'function') navigate(defaultPage);
                    else goHomeOnce();
                }, 100);
            }
        }
    ...
    function persistLoginCheck() {
        ...
        if (sessionStorage.getItem('axis_force_login_session') === '1') {
            ...
            if (mn) { mn.style.display = 'none'; mn.style.opacity = '0'; mn.style.visibility = 'hidden'; }
            currentUser = null;
        ...
        var au = document.getElementById('auth-screen');
        var mn = document.getElementById('main-content');
        var introOl = document.getElementById('intro-axis-overlay');
        if (!au || !mn) return;
        var loginPersist = (localStorage.getItem('current_user_login') || '').trim() || (localStorage.getItem('current_user') || '').toLowerCase().replace(/\s+/g, '_');
        function showMainPersist() {
            ...
            if (introOl) introOl.style.display = 'none';
            if (typeof window.axisRobotMaybeShow === 'function') try { window.axisRobotMaybeShow(); } catch (_) {}
            setTimeout(function() { if (typeof window.axisRobotMaybeShow === 'function') try { window.axisRobotMaybeShow(); } catch (_) {} }, 400);
            ...
            if (ma) {
                ...
                var key = 'db_' + (typeof axisLoginCanonico === 'function' ? axisLoginCanonico(login || currentUser || '') : (login || (currentUser || '').toLowerCase().replace(/\s+/g, '_')));
                try {
                    ...
                    if (d && d.perfil === 'admin') {
                        ...
                        currentUserProfile = (d && d.perfil) ? d.perfil : 'operador';
            ...
            var targetSection = document.getElementById(sp);
            if (targetSection) {
    ...
    function ensureActiveSectionVisible() {
        ...
        if (section && section.classList.contains('main-section')) {
            ...
            if (pageId === 'page-home') {
                ...
                var ud = document.getElementById('user-display-name');
                if (ud && (localStorage.getItem('current_user') || window.currentUser)) ud.innerText = (localStorage.getItem('current_user') || window.currentUser || 'Usuário');
                if (typeof atualizarPerfilNav === 'function') atualizarPerfilNav();
...
function navigate(pageId) {
    ...
    switch(pageId) {
        ...
        case 'page-suporte':
            carregarTickets();
            break;
        case 'page-documentacao':
            loadDocumentacao();
            break;
        case 'page-administracao':
            if (currentUserProfile !== 'admin') {
                if (typeof showToast === 'function') showToast('Acesso negado. Apenas administradores podem acessar esta página.', 'error');
                navigate('page-home');
                return;
            }
            if (typeof atualizarEstatisticasAdmin === 'function') atualizarEstatisticasAdmin();
...
function formatHomeDateTime() {
    var d = new Date();
    var tz = 'America/Sao_Paulo'; // Horário de Brasília
    var dateOpts = { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: tz };
    var timeOpts = { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: tz };
    try {
        var dateStr = d.toLocaleDateString('pt-BR', dateOpts);
        var timeStr = d.toLocaleTimeString('pt-BR', timeOpts);
        if (dateStr && dateStr.length > 0) dateStr = dateStr.charAt(0).toUpperCase() + dateStr.slice(1);
...
function updateHomeHero() {
    var saluteEl = document.getElementById('home-hero-salute');
    var nameEl = document.getElementById('home-hero-name');
    var datetimeEl = document.getElementById('home-hero-datetime');
    if (saluteEl) saluteEl.textContent = 'BEM-VINDO AO AXIS';
    var userName = (typeof currentUser === 'string' && currentUser) ? currentUser : (localStorage.getItem('current_user') || '');
    if (!userName) {
        var login = localStorage.getItem('current_user_login');
        if (login) {
            try {
                var db = JSON.parse(localStorage.getItem('db_' + login) || '{}');
                userName = db.name || login;
            } catch (_) {}
        }
    }
    if (nameEl) nameEl.textContent = userName || 'usuário';
    if (datetimeEl) datetimeEl.textContent = formatHomeDateTime();
    if (typeof window.axisRobotMaybeShow === 'function') try { window.axisRobotMaybeShow(); } catch (_) {}
}

var homeHeroClockInterval = null;
function startHomeHeroClock() {
    updateHomeHero();
    if (homeHeroClockInterval) clearInterval(homeHeroClockInterval);
    homeHeroClockInterval = setInterval(updateHomeHero, 1000);
}
...
function loadSettings() {
    try {
    // Carrega configurações salvas (valores seguros com fallback)
    const savedTheme = (localStorage.getItem('axis-theme') || 'light').toLowerCase();
    const theme = (savedTheme === 'dark' || savedTheme === 'light') ? savedTheme : 'light';
    const highContrast = localStorage.getItem('axis-high-contrast') === 'true';
    const rawItems = localStorage.getItem('axis-items-per-page') || '15';
    const savedItemsPerPage = /^\d+$/.test(rawItems) ? Math.min(100, Math.max(5, parseInt(rawItems, 10))) : 15;
    const notificationsEnabled = localStorage.getItem('axis-notifications-enabled') !== 'false';
...
function applyFontSize(size) {
    document.body.classList.remove('axis-font-normal', 'axis-font-large', 'axis-font-xlarge');
    document.body.classList.add('axis-font-' + (size || 'normal'));
}
function applyReduceMotion(enable) {
    if (enable) document.body.classList.add('axis-reduce-motion'); else document.body.classList.remove('axis-reduce-motion');
}
function applyAccentColor(color) {
    document.body.setAttribute('data-accent', color || 'blue');
}
function updateAccentColor() {
    const el = document.getElementById('accent-color-setting');
    const v = el ? el.value : 'blue';
    localStorage.setItem('axis-accent-color', v);
    applyAccentColor(v);
    syncAccentColorPicker();
    showToast('Cor de destaque atualizada', 'info');
}

var ACCENT_LABELS = { blue: 'Azul', green: 'Verde', purple: 'Roxo', orange: 'Laranja' };

function syncAccentColorPicker() {
    var sel = document.getElementById('accent-color-setting');
    var swatch = document.getElementById('accent-color-swatch');
    var label = document.getElementById('accent-color-label');
    var options = document.querySelectorAll('.accent-color-option');
    if (!sel || !swatch || !label) return;
    var v = sel.value || 'blue';
    swatch.className = 'accent-color-swatch accent-swatch-' + v;
    label.textContent = ACCENT_LABELS[v] || v;
...
function initAccentColorPicker() {
    if (window._accentColorPickerInit) return;
    var picker = document.getElementById('accent-color-picker');
    var trigger = document.getElementById('accent-color-trigger');
    var dropdown = document.getElementById('accent-color-dropdown');
    var sel = document.getElementById('accent-color-setting');
...
function updateConfigSessionCard() {
    ...
    var descEl = document.getElementById('config-session-user-desc');
    var pageEl = document.getElementById('config-current-page-name');
    var labels = { 'page-home': 'Home', 'page-inventario': 'Inventário', 'page-rondas': 'Rondas', 'page-administracao': 'Administração', 'page-configuracoes': 'Configurações', 'page-suporte': 'Suporte', 'page-documentacao': 'Documentação' };
    if (nameEl) nameEl.textContent = (typeof currentUser !== 'undefined' && currentUser) ? currentUser : '—';
    if (descEl) descEl.textContent = (typeof currentUser !== 'undefined' && currentUser) ? 'Sessão atual' : 'Não conectado';
    if (pageEl) pageEl.textContent = labels[activePage] || activePage || '—';
}
function importSettingsFromFile(ev) {
    ...
    r.onload = function() {
        try {
            ...
            if (data.theme && (data.theme === 'light' || data.theme === 'dark')) { localStorage.setItem('axis-theme', data.theme); if (typeof setTheme === 'function') setTheme(data.theme); }
            if (typeof data.highContrast !== 'undefined') { localStorage.setItem('axis-high-contrast', data.highContrast); document.body.classList.toggle('high-contrast', !!data.highContrast); }
            if (data.itemsPerPage) { var n = parseInt(data.itemsPerPage, 10); if (!isNaN(n) && n >= 5 && n <= 100) { localStorage.setItem('axis-items-per-page', String(n)); } }
            if (typeof data.notificationsEnabled !== 'undefined') localStorage.setItem('axis-notifications-enabled', !!data.notificationsEnabled);
            if (typeof data.notificationsSound !== 'undefined') localStorage.setItem('axis-notifications-sound', !!data.notificationsSound);
            if (data.fontSize && AXIS_VALID_FONT_SIZES.indexOf(data.fontSize) >= 0) localStorage.setItem('axis-font-size', data.fontSize);
            if (typeof data.reduceMotion !== 'undefined') localStorage.setItem('axis-reduce-motion', !!data.reduceMotion);
            if (data.logoutTimeout !== undefined) { var t = parseInt(data.logoutTimeout, 10); if (!isNaN(t) && t >= 0) localStorage.setItem('axis-logout-timeout', String(t)); }
            if (typeof data.maskSensitive !== 'undefined') localStorage.setItem('axis-mask-sensitive', !!data.maskSensitive);
        ...
        } catch (e) { showToast('Arquivo inválido', 'error'); ev.target.value = ''; }
    };
    r.readAsText(f);
}
function resetSettingsToDefault() {
    if (!confirm('Restaurar todas as configurações ao padrão? Suas preferências atuais serão perdidas.')) return;
    localStorage.removeItem('axis-theme');
    localStorage.removeItem('axis-high-contrast');
    localStorage.setItem('axis-items-per-page', '15');
    localStorage.removeItem('axis-notifications-enabled');
    localStorage.removeItem('axis-notifications-sound');
    localStorage.removeItem('axis-font-size');
    localStorage.removeItem('axis-reduce-motion');
    localStorage.setItem('axis-logout-timeout', '0');
    localStorage.removeItem('axis-mask-sensitive');
    localStorage.removeItem('axis-date-format');
    localStorage.removeItem('axis-time-format');
    localStorage.removeItem('axis-accent-color');
    localStorage.removeItem('axis-home-page');
    setTheme('light');
...
function toggleNotifications() {
    const el = document.getElementById('notifications-enabled');
    const enabled = el ? el.checked : true;
    localStorage.setItem('axis-notifications-enabled', enabled);
    showToast(enabled ? 'Notificações ativadas' : 'Notificações desativadas', 'info');
}
function toggleNotificationsSound() {
    const el = document.getElementById('notifications-sound');
    localStorage.setItem('axis-notifications-sound', el ? el.checked : false);
}
function updateFontSize() {
    const el = document.getElementById('font-size-setting');
    const v = el ? el.value : 'normal';
    localStorage.setItem('axis-font-size', v);
    applyFontSize(v);
    showToast('Tamanho da fonte atualizado', 'info');
}
function toggleReduceMotion() {
    const el = document.getElementById('reduce-motion');
    const enable = el ? el.checked : false;
    localStorage.setItem('axis-reduce-motion', enable);
...
function toggleMaskSensitive() {
    ...
    localStorage.setItem('axis-mask-sensitive', el ? el.checked : false);
    showToast('Preferência de mascaramento salva', 'info');
}
function updateDateFormat() {
    const el = document.getElementById('date-format-setting');
    const v = el ? el.value : 'ddmmyyyy';
    localStorage.setItem('axis-date-format', v);
    showToast('Formato de data atualizado', 'info');
}
function updateTimeFormat() {
    const el = document.getElementById('time-format-setting');
...
function startInactivityTimer() {
    ...
    function onActivity() { axisLastActivity = Date.now(); }
...
function clearLocalCache() {
    if (confirm('Tem certeza que deseja limpar o cache local? Isso não afetará os dados do inventário.')) {
        const keysToKeep = ['db_', 'axis-theme', 'axis-high-contrast', 'axis-items-per-page', 'axis-notifications-enabled', 'axis-notifications-sound', 'axis-font-size', 'axis-reduce-motion', 'axis-logout-timeout', 'axis-mask-sensitive', 'axis-date-format', 'axis-time-format', 'axis-accent-color', 'axis-home-page'];
        const keysToRemove = [];
...
function exportSettings() {
    const settings = {
        ...
        notificationsEnabled: localStorage.getItem('axis-notifications-enabled') !== 'false',
        notificationsSound: localStorage.getItem('axis-notifications-sound') === 'true',
        fontSize: localStorage.getItem('axis-font-size') || 'normal',
        reduceMotion: localStorage.getItem('axis-reduce-motion') === 'true',
        logoutTimeout: localStorage.getItem('axis-logout-timeout') || '0',
        maskSensitive: localStorage.getItem('axis-mask-sensitive') === 'true',
...
/** Normalização única do login: usado no cadastro E no login. Todo usuário criado por admin é guardado e encontrado com esta forma. */
function axisLoginCanonico(s) {
    var t = (typeof s === 'string' ? s : '').trim().toLowerCase();
...
function handleAuth() {
    ...
    var loginNormalizado = axisLoginCanonico(userInput);
    var dbKey = 'db_' + loginNormalizado;
    ...
    console.log('📋 Chaves db_ no localStorage:', allKeys.length, allKeys.join(', ') || '(nenhuma)');

    var dbRaw = localStorage.getItem(dbKey);
    var chaveAntiga = null;
    if (!dbRaw) {
        dbRaw = localStorage.getItem('db_' + (userInput.trim().toLowerCase().replace(/\s+/g, '_')));
        if (dbRaw) chaveAntiga = 'db_' + (userInput.trim().toLowerCase().replace(/\s+/g, '_'));
    }
    if (!dbRaw && allKeys.length > 0) {
        for (var i = 0; i < allKeys.length; i++) {
            var k = allKeys[i];
            var parte = axisLoginCanonico(k.replace('db_', ''));
            if (parte === loginNormalizado) {
                chaveAntiga = k;
                dbRaw = localStorage.getItem(k);
                console.log('📦 Usuário encontrado por chave alternativa:', k);
                break;
            }
        }
    }
...
function execLogout() {
    ...
    // Mostra tela de login imediatamente (evita tela branca) e esconde conteúdo
    if (authScreen) {
        authScreen.style.display = 'flex';
        authScreen.style.opacity = '1';
    ...
    // Registra logout e limpa dados
    var logoutData = { usuario: currentUser, data: new Date().toISOString() };
...
function showModalTotpLogin(login, onSuccess) {
    ...
    function submit() {
        ...
        if (code.length !== 6 || !/^\d+$/.test(code)) {
            if (errEl) { errEl.textContent = 'Código deve ter 6 dígitos.'; errEl.style.display = 'block'; }
            return;
        }
...
function showModalTotpReset() {
    var wrap = document.getElementById('modal-totp-reset');
    var input = document.getElementById('totp-reset-code-input');
    var errEl = document.getElementById('totp-reset-err');
    var submitBtn = document.getElementById('totp-reset-submit');
    var cancelBtn = document.getElementById('totp-reset-cancel');
    var overlay = document.getElementById('modal-totp-reset-overlay');
    if (!wrap || !input) return;
    ...
    function submit() {
        var code = input.value.trim();
        if (code.length !== 6 || !/^\d+$/.test(code)) {
            errEl.textContent = 'Introduza o código de 6 dígitos do app.';
        ...
        fetch(apiBase.replace(/\/+$/, '') + '/api/auth/totp-reset', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ login: getTotpLoginNormalized(), code: code })
        }).then(function(r) { return r.json().then(function(d) { return { ok: r.ok, data: d }; }).catch(function() { return { ok: false, data: null }; }); })
          .then(function(result) {
              submitBtn.disabled = false;
              if (result.ok && result.data && result.data.ok) {
                  close();
                  if (typeof showToast === 'function') showToast('2FA redefinido. No próximo login será solicitado o código.', 'success');
                  if (typeof refreshTotpSettings === 'function') refreshTotpSettings();
              } else {
                  errEl.textContent = (result.data && result.data.error) || 'Código inválido. Use o código que está AGORA no app.';
                  errEl.style.display = 'block';
              }
          })
          .catch(function() {
...
function fecharModalTotpReset() {
    var wrap = document.getElementById('modal-totp-reset');
    if (wrap) wrap.style.display = 'none';
}
window.showModalTotpReset = showModalTotpReset;
window.fecharModalTotpReset = fecharModalTotpReset;

// --- Google Authenticator: funções globais para o botão funcionar sempre ---
function showTotpSetupAreaWithQr(otpauthUrl, secret) {
    var area = document.getElementById('totp-setup-area');
    var qrWrap = document.getElementById('totp-qr-wrap');
    var secretEl = document.getElementById('totp-secret-display');
    if (!area) return;
    area.style.display = 'block';
    area.style.visibility = 'visible';
    area.style.opacity = '1';
    if (qrWrap && otpauthUrl) {
        var encoded = encodeURIComponent(otpauthUrl);
        var url1 = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' + encoded;
        var url2 = 'https://quickchart.io/qr?text=' + encoded + '&size=200';
        var img = document.createElement('img');
        img.alt = 'QR Code Google Authenticator';
        ...
        img.onerror = function() {
            ...
            img.onerror = function() {
                ...
                if (qrWrap) qrWrap.innerHTML = '<p style="color:var(--text-secondary);">Use o código manual abaixo no app.</p>';
            };
        };
        img.onload = function() {
            if (qrWrap) { qrWrap.innerHTML = ''; qrWrap.appendChild(img); }
        };
        qrWrap.innerHTML = '';
        qrWrap.appendChild(img);
    } else if (qrWrap) {
        qrWrap.innerHTML = '<p style="color:var(--warning-color);">QR indisponível. Use o código manual.</p>';
    }
    if (secretEl) secretEl.textContent = secret || '';
    var errP = document.getElementById('totp-confirm-error');
    if (errP) { errP.style.display = 'none'; errP.textContent = ''; }
    var input = document.getElementById('totp-confirm-input');
    if (input) { input.value = ''; setTimeout(function() { input.focus(); }, 300); }
    try { area.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) { area.scrollIntoView(true); }
    var card = document.getElementById('totp-settings-card');
    if (card) try { card.scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (_) {}
}
function showTotpSetupAreaError(message, showRetry) {
    var area = document.getElementById('totp-setup-area');
    var qrWrap = document.getElementById('totp-qr-wrap');
    if (!area) return;
    area.style.display = 'block';
    area.style.visibility = 'visible';
    if (qrWrap) qrWrap.innerHTML = '<p style="color:var(--danger-color);margin:12px 0;">' + (message || 'Erro ao obter QR code.') + '</p>' +
        (showRetry ? '<button type="button" class="axis-btn totp-btn-retry" id="totp-retry-btn">TENTAR NOVAMENTE</button>' : '');
    try { area.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) {}
    if (showRetry) {
        setTimeout(function() {
            var retryBtn = document.getElementById('totp-retry-btn');
            if (retryBtn) retryBtn.onclick = function() { if (window.runTotpSetup) window.runTotpSetup(); };
        }, 100);
    }
}
function runTotpSetup() {
    var currentLogin = getTotpLoginNormalized();
    if (!currentLogin) {
        if (typeof showToast === 'function') showToast('Sessão inválida. Faça login novamente.', 'warning');
        return;
    }
    var area = document.getElementById('totp-setup-area');
    var qrWrap = document.getElementById('totp-qr-wrap');
    if (area) {
        area.style.display = 'block';
        area.style.visibility = 'visible';
        if (qrWrap) qrWrap.innerHTML = '<p style="color:var(--text-secondary);">A carregar QR code...</p>';
        try { area.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) {}
    }
    if (typeof showToast === 'function') showToast('A abrir configuração 2FA...', 'info');
    ...
    var setupUrl = apiBase.replace(/\/+$/, '') + '/api/auth/totp-setup';
    fetch(setupUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        if (!r.ok) {
            return r.text().then(function(t) { throw new Error('API ' + r.status + (t ? ': ' + t.substring(0, 80) : '')); });
        }
        if (contentType.indexOf('application/json') !== -1) return r.json();
        return r.text().then(function(t) { try { return JSON.parse(t); } catch (_) { throw new Error('Resposta inválida'); } });
    }).then(function(data) {
        if (!data) { showTotpSetupAreaError('Resposta vazia do servidor.', true); return; }
        var otpauthUrl = data.otpauthUrl || data.otpauth_url;
        var secret = data.secret || '';
        if (!otpauthUrl && !secret) { showTotpSetupAreaError('Resposta sem dados para o QR. Reinicie o servidor e tente de novo.', true); return; }
        showTotpSetupAreaWithQr(otpauthUrl, secret);
        if (typeof showToast === 'function') showToast('QR code pronto. Escaneie com o app.', 'success');
    }).catch(function(err) {
        var msg = (err && err.message) ? err.message : 'Erro de rede ou servidor.';
        if (typeof showToast === 'function') showToast(msg, 'error');
        var displayMsg = msg.indexOf('API ') === 0 ? 'Resposta do servidor: ' + msg : 'Não foi possível ligar ao servidor. Certifique-se de que o AXIS está a correr (janela do Node aberta) e use «Abrir AXIS (como administrador).bat» se precisar. Clique em TENTAR NOVAMENTE após confirmar que o servidor está ativo.';
        showTotpSetupAreaError(displayMsg, true);
    });
}
window.runTotpSetup = runTotpSetup;

function runTotpSetupForceNew() {
    var currentLogin = getTotpLoginNormalized();
    if (!currentLogin) {
        if (typeof showToast === 'function') showToast('Sessão inválida. Faça login novamente.', 'warning');
        return;
    }
    if (typeof showToast === 'function') showToast('A gerar novo código... Remova a entrada AXIS antiga no app.', 'info');
    var area = document.getElementById('totp-setup-area');
    var qrWrap = document.getElementById('totp-qr-wrap');
    if (area) { area.style.display = 'block'; area.style.visibility = 'visible'; }
    if (qrWrap) qrWrap.innerHTML = '<p style="color:var(--text-secondary);">A gerar novo QR...</p>';
    var apiBase = (typeof getAxisApiBase === 'function' ? getAxisApiBase() : null) || window.location.origin || 'http://localhost:3006';
    if (!/^https?:\/\//i.test(apiBase)) apiBase = 'http://localhost:3006';
    fetch(apiBase.replace(/\/+$/, '') + '/api/auth/totp-setup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ login: currentLogin, forceNew: true })
    }).then(function(r) {
        if (!r.ok) return r.text().then(function(t) { throw new Error('API ' + r.status); });
        return r.json();
    }).then(function(data) {
        if (!data || (!data.otpauthUrl && !data.otpauth_url && !data.secret)) {
            showTotpSetupAreaError('Resposta inválida.', true);
            return;
        }
        var otpauthUrl = data.otpauthUrl || data.otpauth_url;
        var secret = data.secret || '';
        showTotpSetupAreaWithQr(otpauthUrl, secret);
        if (typeof showToast === 'function') showToast('Novo QR gerado. Escaneie no app (apague a entrada AXIS antiga primeiro).', 'success');
    }).catch(function(err) {
        var msg = (err && err.message) ? err.message : 'Erro de rede.';
        showTotpSetupAreaError(msg.indexOf('API ') === 0 ? msg : 'Erro ao ligar ao servidor. Verifique se o AXIS está a correr e clique em TENTAR NOVAMENTE.', true);
    });
}
window.runTotpSetupForceNew = runTotpSetupForceNew;

// Confirmar 2FA: expor globalmente para o botão CONFIRMAR E ATIVAR funcionar sempre (evita múltiplos envios)
var _totpConfirmInFlight = false;
function confirmarTotp2FA() {
    if (_totpConfirmInFlight) return;
    if (typeof refreshTotpSettings !== 'function') return;
    var confirmBtn = document.getElementById('totp-confirm-btn');
    var currentLogin = getTotpLoginNormalized();
    var code = (document.getElementById('totp-confirm-input') || {}).value.trim();
    if (!currentLogin) {
        if (typeof showToast === 'function') showToast('Sessão inválida. Faça login novamente.', 'warning');
        return;
    }
    if (code.length !== 6 || !/^\d+$/.test(code)) {
        if (typeof showToast === 'function') showToast('Introduza o código de 6 dígitos', 'warning');
        return;
    }
    var errP = document.getElementById('totp-confirm-error');
    if (errP) { errP.style.display = 'none'; errP.textContent = ''; }
    ...
    fetch(confirmUrl, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ login: currentLogin, code: code })
    }).then(function(r) {
        return r.json().then(function(data) { return { ok: r.ok, data: data }; }).catch(function() { return { ok: false, data: null }; });
    }).then(function(result) {
        _totpConfirmInFlight = false;
        if (confirmBtn) { confirmBtn.disabled = false; confirmBtn.textContent = 'CONFIRMAR E ATIVAR'; }
        if (result.ok && result.data && result.data.ok) {
            if (errP) errP.style.display = 'none';
            if (typeof showToast === 'function') showToast('2FA ativado com sucesso', 'success');
            try { localStorage.setItem('axis_2fa_enabled', '1'); } catch (_) {}
            update2FABadge(true);
            var area = document.getElementById('totp-setup-area');
            if (area) area.style.display = 'none';
            var activeMsg = document.getElementById('totp-active-message');
            if (activeMsg) activeMsg.style.display = 'block';
            refreshTotpSettings();
        } else {
            if (errP) { errP.textContent = (result.data && result.data.error) ? result.data.error : 'Código inválido. Use o código que está AGORA no app (renova a cada 30 s).'; errP.style.display = 'block'; }
            if (typeof showToast === 'function') showToast((result.data && result.data.error) || 'Código inválido.', 'error');
        }
    }).catch(function() {
        _totpConfirmInFlight = false;
        if (confirmBtn) { confirmBtn.disabled = false; confirmBtn.textContent = 'CONFIRMAR E ATIVAR'; }
        if (errP) { errP.textContent = 'Erro de ligação. Verifique se a janela do Node (servidor AXIS) está aberta e clique em CONFIRMAR E ATIVAR novamente.'; errP.style.display = 'block'; }
        if (typeof showToast === 'function') showToast('Erro de ligação. Tente novamente.', 'error');
    });
}
window.confirmarTotp2FA = confirmarTotp2FA;

function update2FABadge(show) {
    var badge = document.getElementById('nav-2fa-badge');
    if (!badge) return;
    if (show === undefined) show = localStorage.getItem('axis_2fa_enabled') === '1';
...
function bindTotpEnableButton() {
    ...
    if (btn && window.runTotpSetup && !btn.getAttribute('data-totp-bound')) {
        btn.setAttribute('data-totp-bound', '1');
        btn.addEventListener('click', function(e) { e.preventDefault(); window.runTotpSetup(); });
    }
}
document.addEventListener('DOMContentLoaded', bindTotpEnableButton);
window.addEventListener('load', function() { setTimeout(bindTotpEnableButton, 200); });

var _totpSettingsBound = false;
function refreshTotpSettings() {
    var login = getTotpLoginNormalized();
    var statusEl = document.getElementById('totp-status-text');
    var enableBtn = document.getElementById('totp-enable-btn');
    var resetBtn = document.getElementById('totp-reset-btn');
    var setupArea = document.getElementById('totp-setup-area');
    if (!statusEl || !enableBtn || !resetBtn) return;
    // Mostrar estado e botão logo (evita secção vazia enquanto a API responde)
    if (!login) {
        statusEl.textContent = 'Inicie sessão para configurar 2FA.';
        enableBtn.style.display = 'none';
        resetBtn.style.display = 'none';
        return;
    }
    statusEl.textContent = '2FA desativado';
    ...
    fetch(apiBase.replace(/\/+$/, '') + '/api/auth/totp-status?login=' + encodeURIComponent(login))
        ...
        .then(function(data) {
            if (data && data.enabled) {
                statusEl.textContent = '2FA ativado';
                statusEl.classList.add('totp-status-active');
                enableBtn.style.display = 'none';
                ...
                statusEl.textContent = '2FA desativado';
                statusEl.classList.remove('totp-status-active');
                enableBtn.style.display = 'inline-block';
                resetBtn.style.display = 'none';
                try { localStorage.removeItem('axis_2fa_enabled'); } catch (_) {}
                var activeMsg = document.getElementById('totp-active-message');
                if (activeMsg) activeMsg.style.display = 'none';
                update2FABadge(false);
            }
            if (setupArea) setupArea.style.display = 'none';
        });

    enableBtn.onclick = function() { if (window.runTotpSetup) window.runTotpSetup(); };

    if (!_totpSettingsBound) {
        _totpSettingsBound = true;
        var confirmBtn = document.getElementById('totp-confirm-btn');
        if (confirmBtn) confirmBtn.onclick = function() { if (window.confirmarTotp2FA) window.confirmarTotp2FA(); };
        var cancelBtn = document.getElementById('totp-setup-cancel');
        if (cancelBtn) cancelBtn.onclick = function() {
            var area = document.getElementById('totp-setup-area');
            if (area) area.style.display = 'none';
        };
        if (resetBtn) resetBtn.onclick = function() {
            if (typeof showModalTotpReset === 'function') showModalTotpReset();
        };
    }
    if (typeof update2FABadge === 'function') update2FABadge();
}
...
window.showModalTotpLogin = showModalTotpLogin;
window.fecharModalTotpLogin = fecharModalTotpLogin;
...
function inicializarInventario() {
    ...
    // Re-lê do localStorage para ter dados atualizados (evita precisar recarregar a página)
    try {
        var s = localStorage.getItem('axis_inventario_equipamentos');
        if (s) {
            var arr = JSON.parse(s);
    ...
    // Aplica filtro "Todos" para exibir todos os dados imediatamente
    if (typeof filtrarInventario === 'function') {
        filtrarInventario();
    } else {
        inventarioData = Array.isArray(equipamentosExemplo) ? [...equipamentosExemplo] : [];
        renderizarTabela();
    }
...
function filtrarInventario() {
    ...
    var modeloPanel = (document.getElementById('ucs-filtro-panel-modelo')?.value || '').trim();
    var setorPanel = (document.getElementById('ucs-filtro-panel-setor')?.value || '').trim();
    const busca = '';
    // "Todos" ou vazio = não filtrar por modelo/setor
    if (modeloPanel.toLowerCase() === 'todos') modeloPanel = '';
    if (setorPanel.toLowerCase() === 'todos') setorPanel = '';
...
function limparFormularioUsuario() {
    ...
    if (typeof atualizarPerfilSelectCadastro === 'function') atualizarPerfilSelectCadastro();
}
...
function atualizarPerfilSelectCadastro() {
    const select = document.getElementById('novo-usuario-perfil');
    const valueEl = document.getElementById('perfil-select-value-cadastro');
    if (!select || !valueEl) return;
    const opt = select.options[select.selectedIndex];
    valueEl.textContent = opt ? opt.textContent : 'Selecione um perfil';
    valueEl.classList.toggle('placeholder', !select.value);
    const dropdown = document.getElementById('perfil-select-dropdown-cadastro');
    if (dropdown) {
        dropdown.querySelectorAll('.perfil-select-option').forEach(o => {
            o.classList.toggle('selected', o.getAttribute('data-value') === select.value);
        });
    }
}

function initPerfilSelectCadastro() {
    const wrap = document.getElementById('perfil-select-wrap-cadastro');
    if (!wrap) return;
    const select = document.getElementById('novo-usuario-perfil');
    const trigger = document.getElementById('perfil-select-trigger-cadastro');
    const dropdown = document.getElementById('perfil-select-dropdown-cadastro');
    if (!select || !trigger || !dropdown) return;
    ...
    document.addEventListener('click', function fecharPerfilDropdownCadastro(e) {
...
function atualizarSetorSelectEditar() {
    const select = document.getElementById('editar-usuario-setor');
    const valueEl = document.getElementById('setor-select-value-editar');
    if (!select || !valueEl) return;
    const opt = select.options[select.selectedIndex];
    valueEl.textContent = opt ? opt.textContent : 'Selecione um setor';
    valueEl.classList.toggle('placeholder', !select.value);
    const dropdown = document.getElementById('setor-select-dropdown-editar');
    if (dropdown) {
        dropdown.querySelectorAll('.perfil-select-option').forEach(o => {
            o.classList.toggle('selected', o.getAttribute('data-value') === select.value);
        });
...
function initSetorSelectEditar() {
    const wrap = document.getElementById('setor-select-wrap-editar');
    if (!wrap) return;
    const select = document.getElementById('editar-usuario-setor');
    const trigger = document.getElementById('setor-select-trigger-editar');
    const dropdown = document.getElementById('setor-select-dropdown-editar');
    const valueEl = document.getElementById('setor-select-value-editar');
    if (!select || !trigger || !dropdown) return;
    ...
    document.addEventListener('click', function fecharSetorDropdown(e) {
        if (!wrap.contains(e.target) && !dropdown.contains(e.target)) {
            dropdown.classList.remove('open');
            trigger.setAttribute('aria-expanded', 'false');
            dropdown.setAttribute('aria-hidden', 'true');
...
function salvarUsuarioEditado(event) {
    ...
    var login = (document.getElementById('editar-usuario-login') && document.getElementById('editar-usuario-login').value) || '';
    var loginNorm = axisLoginCanonico(login);
    var originalKey = (document.getElementById('editar-usuario-original-key') && document.getElementById('editar-usuario-original-key').value) || ('db_' + login);
...
function abrirDocumentacao() {
    ...
    navigate('page-documentacao');
}

// ================= DOCUMENTAÇÃO AXIS =================
// Documentos armazenados no servidor (suporte 2TB+ via DOCS_STORAGE_DIR)
const DOC_STORAGE_KEY = 'axis_documentacao';
function getDocApiBase() {
    if (typeof window !== 'undefined' && window.location && window.location.origin) {
        return window.location.origin + '/api/docs';
    }
    return '/api/docs';
}
const DOC_API_BASE = '/api/docs';
const DOCS_POR_PAGINA = 24;
...
async function fetchDocumentos() {
    try {
        const r = await fetch(getDocApiBase());
        if (!r.ok) return [];
        const j = await r.json();
        if (j && j.ok && Array.isArray(j.docs)) return j.docs;
        return [];
    } catch (e) {
        return [];
    }
...
async function getDocumentos() {
    if (documentosCacheLoaded) return documentosCache;
    documentosCache = await fetchDocumentos();
    documentosCacheLoaded = true;
        ...
        if (Array.isArray(loc) && loc.length > 0) {
            for (const d of loc) {
                if (d.tipo === 'texto' && d.conteudo) {
                    try {
                        await fetch(getDocApiBase(), {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ titulo: d.titulo, descricao: d.descricao || '', categoria: d.categoria || 'documentos', conteudo: d.conteudo })
                        });
                    } catch (_) {}
                }
            }
...
function invalidarCacheDocs() {
    documentosCacheLoaded = false;
    documentosCache = [];
}
...
async function renderizarDocumentos() {
    ...
    const tabAtivo = document.querySelector('.doc-tab.active');
    const categoriaFiltro = tabAtivo ? tabAtivo.getAttribute('data-categoria') : 'todos';
    const buscaInput = document.getElementById('doc-busca');
    ...
    if (busca) filtrados = filtrados.filter(d =>
        ...
        ((d.conteudo || '').substring(0, 500)).toLowerCase().includes(busca)
    );
    ...
    if (filtrados.length === 0) {
        ...
        if (empty) {
            ...
            if (h3) h3.textContent = busca || categoriaFiltro !== 'todos' ? 'Nenhum documento encontrado' : 'Nenhum documento cadastrado';
        }
        return;
    }
    ...
    filtrados.forEach(doc => {
        ...
        card.innerHTML = `
            ...
            <div class="doc-item-header">
                <span class="doc-item-categoria">${label}</span>
                <div class="doc-item-actions">
                    <button class="doc-btn-icon" onclick="event.stopPropagation(); editarDocumento('${idSafe(doc.id)}')" title="Editar">✏️</button>
                    <button class="doc-btn-icon" onclick="event.stopPropagation(); excluirDocumento('${idSafe(doc.id)}')" title="Excluir">🗑️</button>
                </div>
            </div>
            <h4 class="doc-item-titulo">${(doc.titulo || '').escapeHtml()}</h4>
            <p class="doc-item-desc">${(desc || '').escapeHtml()}</p>
            <button class="doc-item-btn" onclick="event.stopPropagation(); verDocumento('${idSafe(doc.id)}')">Ver documento</button>
        `;
        card.onclick = (e) => { if (!e.target.closest('.doc-item-actions') && !e.target.closest('.doc-item-btn')) verDocumento(doc.id); };
        lista.appendChild(card);
    });
}

async function abrirModalDocumento(editId) {
    ...
    form.reset();
    if (typeof resetDocFileZone === 'function') resetDocFileZone();
    document.getElementById('doc-id').value = editId || '';
    document.getElementById('doc-tipo').value = 'arquivo';
    const arqGrp = document.querySelector('.doc-arquivo-group');
    const txtGrp = document.querySelector('.doc-texto-group');
    if (arqGrp) arqGrp.style.display = 'block';
    if (txtGrp) txtGrp.style.display = 'none';
    if (arqInput) arqInput.required = true;
    title.textContent = editId ? 'Editar documento' : 'Adicionar documento';
    if (editId) {
        const docs = await getDocumentos();
        ...
        if (doc) {
            ...
            if (txtGrp) txtGrp.style.display = doc.tipo === 'texto' ? 'block' : 'none';
            if (arqInput) arqInput.required = false;
        }
    }
    modal.style.display = 'flex';
}
...
function docToggleTipo() {
    ...
    const arqGrp = document.querySelector('.doc-arquivo-group');
    const textoGrp = document.querySelector('.doc-texto-group');
    const arqInput = document.getElementById('doc-arquivo');
    if (!tipo) return;
    if (tipo.value === 'texto') {
        if (arqGrp) arqGrp.style.display = 'none';
        if (textoGrp) textoGrp.style.display = 'block';
        if (arqInput) arqInput.required = false;
    } else {
        if (arqGrp) arqGrp.style.display = 'block';
        if (textoGrp) textoGrp.style.display = 'none';
        if (arqInput) arqInput.required = true;
    }
}
...
function initDocModal() {
    ...
    if (tipo && !tipo.dataset.inited) {
        ...
        tipo.addEventListener('change', docToggleTipo);
    }
...
function initDocBusca() {
    const busca = document.getElementById('doc-busca');
    if (busca && !busca.dataset.docInited) {
        busca.dataset.docInited = '1';
        busca.addEventListener('input', () => renderizarDocumentos());
    }
}
...
async function salvarDocumento(e) {
    ...
    if (!id) {
        if (tipo === 'arquivo' && (!arqInput || !arqInput.files || !arqInput.files[0])) {
            showToast('Selecione um arquivo para enviar.', 'error');
            return;
        }
        if (tipo === 'texto' && !conteudo) {
            showToast('Informe o conteúdo do documento.', 'error');
            return;
        }
    }
    ...
    try {
        if (id) {
            const r = await fetch(getDocApiBase() + '/' + encodeURIComponent(id), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ titulo, descricao, categoria, conteudo })
            });
            const j = await r.json();
            if (!j.ok) throw new Error(j.error || 'Erro ao atualizar');
        } else if (tipo === 'arquivo' && arqInput.files[0]) {
            const formData = new FormData();
            formData.append('titulo', titulo);
            formData.append('descricao', descricao);
            formData.append('categoria', categoria);
            formData.append('arquivo', arqInput.files[0]);
            const r = await fetch(getDocApiBase(), { method: 'POST', body: formData });
            let j;
            try {
                const txt = await r.text();
                j = txt ? JSON.parse(txt) : {};
            } catch (_) {
                throw new Error(r.status === 404 ? 'Serviço de documentação não disponível. Feche todas as janelas do servidor (Node), execute start.bat e abra o endereço que aparecer (ex: http://localhost:3007).' : 'Resposta inválida do servidor (' + r.status + ').');
            }
        ...
        showToast(id ? 'Documento atualizado.' : 'Documento adicionado.', 'success');
    } catch (err) {
...
async function verDocumento(id) {
    ...
    const tituloEl = document.getElementById('doc-view-titulo');
    const descEl = document.getElementById('doc-view-descricao');
    const arquivoArea = document.getElementById('doc-view-arquivo-area');
...
if (typeof axisLoginCanonico === 'function') {
    window.axisLoginCanonico = axisLoginCanonico;
}
...
if (typeof loadDocumentacao === 'function') window.loadDocumentacao = loadDocumentacao;
if (typeof abrirModalDocumento === 'function') window.abrirModalDocumento = abrirModalDocumento;
if (typeof fecharModalDocumento === 'function') window.fecharModalDocumento = fecharModalDocumento;
if (typeof salvarDocumento === 'function') window.salvarDocumento = salvarDocumento;
if (typeof verDocumento === 'function') window.verDocumento = verDocumento;
if (typeof editarDocumento === 'function') window.editarDocumento = editarDocumento;
if (typeof excluirDocumento === 'function') window.excluirDocumento = excluirDocumento;
if (typeof fecharModalDocView === 'function') window.fecharModalDocView = fecharModalDocView;
...
document.addEventListener('click', function(e) {
    ...
    var wrap = btn ? btn.closest('.nav-notifications-wrap') : null;
...
function atualizarPerfilNav() {
    ...
    if (login === 'admin_filipe_silva') {
        ...
        if (dias > 0) diasTexto = 'Faltam ' + dias + ' dias para a troca da senha';
        else if (dias === 0) diasTexto = 'Troca da senha hoje';
        else diasTexto = 'Senha expirada';
    }
...
function abrirPerfilNav() {
    ...
    var overlay = document.getElementById('axis-profile-overlay');
    /* BLINDADO: overlay + modal-center centralizam o card do perfil com fundo borrado. Não alterar sem autorização. Ver BLINDAGEM_MODAL_PERFIL_FOTO.mdc */
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'axis-profile-overlay';
        overlay.className = 'axis-profile-overlay';
        overlay.setAttribute('aria-hidden', 'true');
        overlay.style.display = 'none';
        var backdrop = document.createElement('div');
        backdrop.id = 'axis-profile-overlay-backdrop';
        backdrop.className = 'axis-profile-overlay-backdrop';
        backdrop.setAttribute('aria-label', 'Fechar perfil');
        backdrop.onclick = function() { if (window.fecharPerfilNav) window.fecharPerfilNav(); };
        var center = document.createElement('div');
        center.id = 'axis-profile-modal-center';
        center.className = 'axis-profile-modal-center';
        overlay.appendChild(backdrop);
        overlay.appendChild(center);
        document.body.appendChild(overlay);
    }
    var modalCenter = document.getElementById('axis-profile-modal-center');
    var wrap = document.getElementById('axis-profile-wrap');
    if (overlay && modalCenter) {
        if (wrap && dropdown.parentNode !== modalCenter) {
            modalCenter.appendChild(dropdown);
        }
        overlay.style.display = 'flex';
...
function initAxisProfile() {
    ...
    var photoClick = document.getElementById('axis-profile-photo-click');
    if (photoClick) {
        photoClick.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            if (window.abrirModalFotoPerfil) window.abrirModalFotoPerfil();
        });
    }
...
function abrirModalFotoPerfil() {
    var modal = document.getElementById('axis-profile-photo-modal');
    var imgEl = document.getElementById('axis-profile-photo-modal-img');
    var initialEl = document.getElementById('axis-profile-photo-modal-initial');
    if (!modal || !imgEl) return;
    /* BLINDADO: mover modal para body garante card centralizado (fixed relativo à viewport). Não remover. Ver .cursor/rules/BLINDAGEM_MODAL_PERFIL_FOTO.mdc */
    ...
    if (foto) {
        imgEl.src = foto;
        imgEl.style.display = 'block';
        if (initialEl) { initialEl.style.display = 'none'; initialEl.textContent = initial; }
    } else {
        imgEl.removeAttribute('src');
        imgEl.style.display = 'none';
        if (initialEl) { initialEl.style.display = 'flex'; initialEl.textContent = initial; }
    }
    modal.style.display = 'flex';
    document.body.classList.add('axis-modal-foto-open');
}
function fecharModalFotoPerfil() {
    var modal = document.getElementById('axis-profile-photo-modal');
    if (modal) { modal.style.display = 'none'; document.body.classList.remove('axis-modal-foto-open'); }
}
window.abrirModalFotoPerfil = abrirModalFotoPerfil;
window.fecharModalFotoPerfil = fecharModalFotoPerfil;

function initModalFotoPerfil() {
    var modal = document.getElementById('axis-profile-photo-modal');
    if (!modal) return;
    var btnAtualizar = document.getElementById('axis-profile-photo-btn-atualizar');
    var btnVisualizar = document.getElementById('axis-profile-photo-btn-visualizar');
    var btnSalvar = document.getElementById('axis-profile-photo-btn-salvar');
    var photoInput = document.getElementById('axis-profile-photo-input');
    ...
    modal.querySelectorAll('.axis-profile-photo-modal-overlay, .axis-profile-photo-modal-close').forEach(function(el) {
        if (el && !el._bound) { el._bound = true; el.addEventListener('click', fecharModalFotoPerfil); }
    });
}
