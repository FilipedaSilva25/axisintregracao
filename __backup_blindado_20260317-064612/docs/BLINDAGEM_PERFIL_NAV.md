...
A mensagem *"Uncaught (in promise) Error: A listener indicated an asynchronous response by returning true, but the message channel closed before a response was received"* costuma vir de **extensão do Chrome**, não do AXIS. No `script.js` foi adicionado um handler de `unhandledrejection` que suprime este erro no console, para não poluir a consola. O perfil funciona normalmente; se quiser, pode também desativar extensões em `localhost:3006`.
