 ...
 * Tabela: coluna AÇÕES = Ver | Editar | Excluir (botão Ver à esquerda). Coluna OBSERVAÇÃO = só texto.
...
function renderTabela() {
    ...
    list.forEach(c => {
        ...
        var obsRaw = c.observacao != null ? String(c.observacao) : '';
        var obsPreview = obsRaw ? (obsRaw.length > 50 ? obsRaw.substring(0, 50) + '…' : obsRaw) : '—';
        var obsPreviewSafe = escapeHtml(obsPreview);
        ...
        html += '<tr data-id="' + escapeHtml(c.id || '') + '">' +
            ...
            '<td class="rc-obs-cell"><span class="rc-obs-preview" title="' + obsPreviewSafe + '">' + obsPreviewSafe + '</span></td>' +
            '<td class="rc-actions-cell">' +
            '<button type="button" class="rc-btn-ver-obs" data-id="' + escapeHtml(c.id || '') + '" title="Ver observação completa"><i class="fas fa-eye"></i> Ver</button> ' +
    ...
    tbody.querySelectorAll('.rc-btn-ver-obs').forEach(btn => {
        btn.addEventListener('click', function () {
            var id = this.dataset.id;
            if (!id) return;
            var chamado = getChamados().find(function (c) { return c && c.id === id; });
            var obs = chamado && chamado.observacao != null ? String(chamado.observacao) : '';
            openModalObs(obs);
        });
    });
}
...
function syncFromServer() {
    ...
    fetch(base + '/api/registro-chamados')
        ...
        .then(function (j) {
            if (j && j.ok && Array.isArray(j.chamados)) {
                try {
                    localStorage.setItem(STORAGE_KEY, JSON.stringify(j.chamados));
                    renderTabela();
                } catch (e) {}
            }
        })
...
function init() {
    syncFromServer();
    if (!RC_SYNC_INTERVAL) {
        RC_SYNC_INTERVAL = setInterval(syncFromServer, 15000);
    }
