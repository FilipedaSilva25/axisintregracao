...
function gerarPDFManutencaoFromData(m) {
        ...
        for (var r = 0; r < Math.ceil(titulos.length / 3); r++) {
            ...
            rowTitulos.forEach(function(titulo, colIdx) {
                ...
                itens.forEach(function(c) {
                    ...
                    var boxX = x0 + cardPad, boxY = cy - 2.4, boxS = 2.8;
                    if (c.checked) {
                        doc.setFillColor(COL_GREEN[0], COL_GREEN[1], COL_GREEN[2]);
                        doc.roundedRect(boxX, boxY, boxS, boxS, 0.5, 0.5, 'F');
                        doc.setDrawColor(255, 255, 255);
                        doc.setLineWidth(0.35);
                        doc.line(boxX + 0.5, boxY + 1.4, boxX + 1.15, boxY + 2.05);
                        doc.line(boxX + 1.15, boxY + 2.05, boxX + 2.45, boxY + 0.75);
                    } else {
    ...
    var headerFotoH = 14;
    var fotoBoxH = headerFotoH + fotoGridH + 8;
    var fotoSep = 8;
