...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // PRIMEIRO: Se a URL tiver #login ou ?tela=login = abrir SEMPRE na tela de login (script iniciar-site)
    ...
    if (forceLogin) {
        try {
            ...
            sessionStorage.setItem('axis_force_login_session', '1'); // flag para persistLoginCheck
        } catch (_) {}
        try {
    ...
    if (savedUser && finalIsLoggedIn) {
        ...
        // Usuário já está logado, mostra conteúdo principal imediatamente
        if (authScreen) {
    ...
    function persistLoginCheck() {
        // Sessão aberta pelo script com ?tela=login#login = SEMPRE tela de login (mesmo que URL mude)
        ...
        // Se a URL ainda tiver ?tela=login ou #login, forçar login
        var hash = (window.location.hash || '').toLowerCase();
        var search = (window.location.search || '').toLowerCase();
        if (hash === '#login' || hash.indexOf('#login') === 0 || search.indexOf('tela=login') !== -1) {
            try {
                localStorage.removeItem('user_logged_in');
                localStorage.removeItem('current_user');
                localStorage.removeItem('current_user_login');
...
function startInactivityTimer() {
    ...
    if (axisLogoutTimeoutMinutes <= 0) return;
    ...
    axisLogoutTimeoutId = setInterval(function() {
        ...
        if (elapsed >= axisLogoutTimeoutMinutes) {
            if (axisLogoutTimeoutId) clearInterval(axisLogoutTimeoutId);
            axisLogoutTimeoutId = null;
...
async function fetchDocumentos() {
    try {
        ...
        if (j.ok && Array.isArray(j.docs)) return j.docs;
        return [];
    } catch (e) {
        console.error('Erro ao buscar documentação:', e);
        return [];
    }
}
...
function atualizarPerfilNav() {
    ...
    function setAvatar(el, hasFoto, src) {
        if (!el) return;
        var initialEl = el.nextElementSibling;
        if (hasFoto && src) {
            el.src = src;
            el.style.display = 'block';
            if (initialEl) initialEl.style.display = 'none';
            el.onerror = function() { el.style.display = 'none'; if (initialEl) initialEl.style.display = 'flex'; };
        } else {
    ...
    if (av) setAvatar(av, !!foto, foto);
    if (avDrop) setAvatar(avDrop, !!foto, foto);
