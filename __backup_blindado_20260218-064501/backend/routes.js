...
const fs = require('fs');
const { DATA_DIR, CONFIG_MODULOS, HEADERS, ROOT_DIR, DOCS_STORAGE_DIR, MIME_TYPES } = require('./config');
const formidable = require('formidable');

const MANUTENCOES_DIR = path.join(ROOT_DIR, 'manutencoes');
const DOCS_METADATA_FILE = path.join(DATA_DIR, 'axis-documentacao.json');
...
async function handleApi(req, res, urlPath) {
    ...
    // ---- Raiz: garantir que o site abra sempre na tela de LOGIN ----
    // Quando alguém abre http://localhost:3006/ o Node redireciona para /?tela=login#login
    if (method === 'GET' && (urlPath === '/' || urlPath === '')) {
        const fullUrl = (req.url || '/').split('#')[0];
        const query = (fullUrl.indexOf('?') >= 0) ? fullUrl.substring(fullUrl.indexOf('?') + 1) : '';
        const jaTelaLogin = (query || '').toLowerCase().includes('tela=login');
        if (!jaTelaLogin) {
            res.writeHead(302, {
                'Location': '/?tela=login#login',
                'Content-Type': 'text/html; charset=utf-8'
            });
            res.end('<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0;url=/?tela=login#login"></head><body>Redirecionando para a tela de login...</body></html>', 'utf-8');
            return true;
        }
    }
    ...
    if (docsPath.startsWith('/api/docs')) {
        try {
        const docsDir = DOCS_STORAGE_DIR;
        if (!fs.existsSync(docsDir)) fs.mkdirSync(docsDir, { recursive: true });
        const getDocs = () => {
            try {
                const d = readJsonSync(DOCS_METADATA_FILE, []);
                return Array.isArray(d) ? d : (d.docs || d);
            } catch (e) { return []; }
        };
        const saveDocs = (docs) => { return writeJson(DOCS_METADATA_FILE, docs); };
        const idFromPath = (p) => {
